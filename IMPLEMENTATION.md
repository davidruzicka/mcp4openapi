...
- **Restart-resilient OAuth via encrypted token envelopes**: when `MCP4_OAUTH_KEY` is set, `storeOAuthTokens()` issues `mcp4.v1.*` envelopes (AES-256-GCM, profile_id as AAD) carrying access/refresh/expiry/client_id/scopes/optional creg snapshot; in-memory token maps are keyed by the issued envelope (not the raw IdP access_token); session init detects the `mcp4.v1.` prefix and rehydrates session metadata + DCR client registration directly from the envelope on gateway restart. See `src/auth/token-envelope.ts` and `docs/HTTP-TRANSPORT.md` -> Encrypted Token Envelopes.
