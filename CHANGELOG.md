...
- Per-tool-call audit log (`audit:tool_call` at INFO) and per-upstream / per-client Prometheus dimensions: `mcp_tool_calls_total`, `mcp_tool_call_duration_seconds`, and `mcp_tool_call_errors_total` gain `upstream_host` and `client_identity` labels (capped at 128 / 64 chars); both HTTP and stdio paths emit structured audit records with `sessionId`, `clientPrincipal`, `tool`, `upstreamHost`, `outcome`, and `durationMs` on success, error, and all upstream early-rejects.
