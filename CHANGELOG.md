...
### Fixed
- Grafana profile now splits admin-like reads into `retrieve_admin_content`; `query_metrics` now requires Grafana-style `from`/`to` + `queries`, and user lookup now sends the required `loginOrEmail` query parameter.
