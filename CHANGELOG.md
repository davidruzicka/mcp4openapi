...
## [Unreleased]
### Fixed
- Consent approvals are now stateless: the approval form carries an HMAC-signed token (fingerprint + browser id + expiry, keyed from `MCP4_OAUTH_KEY` with domain separation), so accepting the rules survives a gateway restart between the form render and the click and works across replicas - previously the pending approval lived in one pod's memory and any deploy produced "Consent approval expired". TTL stays `MCP4_CONSENT_APPROVAL_TTL_MS` (default 5 min); one-time consumption is exact per instance and cross-instance replay stays bound to one browser, one request, and the TTL. Without `MCP4_OAUTH_KEY` approvals remain per-instance.
