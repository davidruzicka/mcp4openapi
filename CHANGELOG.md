...
### Added
- Token envelope crypto module (AES-256-GCM, AAD profile binding) for restart-resilient OAuth sessions; wired into HTTP transport in Plan 03.
- HTTP transport now issues encrypted `mcp4.v1.*` token envelopes from `/oauth/token` (auth_code + refresh_token grants) and from internal token refresh when `MCP4_TOKEN_KEY` is set; on initialize, presented envelopes restore session metadata (refresh token, expiry, scopes, client_id, optional client registration) so OAuth clients survive arbitrary gateway restarts. `MCP4_TOKEN_KEY` unset emits a startup warn and preserves byte-identical plain-token behavior. `DEFAULT_MAX_TOKEN_LENGTH` raised from 1000 to 4096 to fit envelope tokens.
