...
## [Unreleased]
### Fixed
- Consent approvals are now stateless: the approval form carries an HMAC-signed token (fingerprint + browser-id digest + expiry, keyed from `MCP4_OAUTH_KEY` with domain separation), so accepting the rules survives a gateway restart between the form render and the click and works across replicas - previously the pending approval lived in one pod's memory and any deploy produced "Consent approval expired". The token embeds only a SHA-256 digest of the browser id, never the cookie value, so the rendered HTML alone cannot be used to forge the cookie binding. TTL stays `MCP4_CONSENT_APPROVAL_TTL_MS` (default 5 min); one-time consumption is exact per instance and cross-instance replay stays bound to one browser, one request, and the TTL (residual risks documented in SECURITY.md). Without `MCP4_OAUTH_KEY` approvals remain per-instance. The unpublished `MCP4_CONSENT_PENDING_MAX` tunable is renamed to `MCP4_CONSENT_CONSUMED_MAX`: it now bounds the consumed-approval replay guard, the pending store no longer exists.
- GitLab profile listing filters actually reach the API: the OpenAPI spec now declares the advertised query parameters for project merge requests (source_branch, target_branch, labels, milestone, author/assignee/reviewer, scope, search, wip, order/sort, updated window and more), project issues, and global issues - previously undeclared filters were silently dropped, so e.g. `list_merge_requests` with `source_branch` returned all merge requests.
- A consent denial caused by a session without a resolvable identity (`no_principal`, typically a pre-restart access token after a gateway restart) now carries `oauth_required` in the JSON-RPC error data, so the HTTP transport answers 401 with a Bearer challenge and OAuth clients silently re-authenticate via their identity-bearing refresh envelope instead of looping on `-32004` while their persisted consent sits unused. Evidence-based denials keep the plain `-32004` + `consent_url` contract.
- The consent approval form's CSP `form-action` now also allows the OAuth client's redirect target (http(s) origin, or a scheme source for custom native-app schemes such as `cursor://`), translated from the redirect_uri already validated by the OAuth layer. Chrome checks `form-action` on every hop of the form submission's redirect chain, so the final redirect delivering the authorization code to the client (e.g. a VS Code loopback listener) was silently aborted and the client never connected.
- OAuth endpoint discovery now works for issuers that carry a path component (Entra: `https://login.microsoftonline.com/{tenant}/v2.0`). The well-known suffix used to be resolved as an absolute URL, which dropped the issuer path, queried the host root, and fell back to nonexistent `${issuer}/oauth/authorize` guesses; discovery now tries the RFC 8414 path-inserted location and then OIDC Discovery appended to the issuer.
...
### Security
- The RFC 6750 `WWW-Authenticate: Bearer`/`resource_metadata` challenge on `/mcp` 401s is scoped to OAuth-active requests: non-OAuth token-auth profiles and degraded/inoperational tenant OAuth configs return their prior plain 401 without an OAuth discovery challenge, so VS Code/Cursor are not lured into an uncompletable OAuth/DCR loop.
- OAuth authorize/token endpoints hardened for RFC 6749 conformance: the token `redirect_uri` must match the value bound at authorize (`invalid_grant` on mismatch), replaying a consumed authorization code revokes the tokens issued during its first redemption, authorization-endpoint errors are delivered as a 302 to the validated `redirect_uri` with `error`/`error_description`/`state` (invalid/unregistered `redirect_uri` or `client_id` stay a direct 400), the shared VS Code proxy `redirect_uri` policy is tightened to https or loopback http only (custom schemes rejected, RFC 8252 loopback ports preserved), `invalid_client` via HTTP Basic returns 401 with `WWW-Authenticate: Basic`, token grant failures map to `invalid_request`/`invalid_grant`/`server_error` (502 upstream) instead of collapsing to `invalid_grant`, `token_type` defaults to `Bearer`, `Cache-Control: no-store` is set on early error responses, unrecognized token parameters are ignored (foreign grant-defining parameters still rejected), live authorization codes and `state` are no longer logged, and a config-time initialization failure no longer permanently 500s all OAuth endpoints.
- PKCE is now mandatory (RFC 7636 / OAuth 2.1): `/oauth/authorize` requires an `S256` `code_challenge` (missing challenge or `plain`/unknown method rejected as `invalid_request`), every authorization code is PKCE-verified at token exchange, `code_verifier` length/charset is enforced (43-128, unreserved set), and any failed verification consumes the single-use code.
- Expired OAuth access tokens now fail closed at the resource server (RFC 6750): per-request expiry is enforced at consumption and, when refresh is unavailable or fails, the request gets `401 invalid_token` with a `WWW-Authenticate: Bearer error="invalid_token", resource_metadata=...` challenge (also added to the expired-envelope 401).
- Resource-server 401 hardening (RFC 6750): every `/mcp` 401 now carries a `WWW-Authenticate: Bearer` challenge with `resource_metadata` and an RFC error code (`invalid_request` for missing/garbled credentials, `invalid_token` for a rejected/expired one) instead of ad-hoc `{error:'Unauthorized'}`; the `Bearer` scheme is matched case-insensitively; a non-OAuth credential (X-API-Token/DRF Token/custom-header) can no longer shadow OAuth by creating an unvalidated session on an OAuth-required profile; a mid-session token replacement is re-validated upstream before it is accepted; the unenforced `scope` attribute is dropped from the challenge; and client-facing error messages are routed through the credential sanitizer.
- Dynamic client registration hardened (RFC 7591 / RFC 8252): `POST /oauth/register` now requires a non-empty `redirect_uris`, validates each redirect URI shape (allows https, loopback http, and custom native-app schemes; rejects `javascript:`, plain remote `http://`, and fragments), returns `400 invalid_redirect_uri`/`invalid_client_metadata` (instead of `500`) for invalid metadata and non-JSON bodies, honors `token_endpoint_auth_method: none` by registering a public client with no secret, and includes `client_id_issued_at` plus `client_secret_expires_at` when a secret is issued.
- OAuth discovery metadata hardened (RFC 8414): authorization-server metadata now advertises `token_endpoint_auth_methods_supported` (`client_secret_basic`/`client_secret_post`/`none`), the advertised `issuer` (and RFC 9728 `resource` origin) tracks the forwarded scheme/host behind a trusted proxy and never emits `http://` for a non-loopback host, and the `/.well-known/openid-configuration` alias is no longer served (the gateway is an OAuth 2.1 AS, not an OpenID Provider; clients use `/.well-known/oauth-authorization-server`).
...
- Client-facing refresh tokens now rotate on every `refresh_token` grant (OAuth 2.1 §4.3.1): each refresh mints a new `mcp4.r1.*` envelope and supersedes the old one, replaying a superseded token fails `invalid_grant` and revokes the whole rotation chain (bounded in-memory state, restart resets to trust-on-first-use), with `rotated`/`reuse_detected` counters (`mcp_oauth_refresh_rotations_total`) and pseudonymized-subject logs.
- Refresh rotation hardened against real-client retry/concurrency without weakening reuse detection: per-session single-flight so concurrent `/mcp` requests share one upstream refresh exchange (no spurious `invalid_grant`/401 flap when the IdP rotates refresh tokens), an idempotent single-flight on `refresh_token` redemption (concurrent or grace-window retry of the just-superseded token returns the already-minted token instead of minting twice or revoking the family), and revoked-family tombstones kept in a separate bounded set the family LRU cap never evicts (a revoked chain can no longer downgrade to trust-on-first-use under cap pressure). Genuine reuse of an older/foreign superseded token still revokes the whole family.
...
- Mid-session token-swap re-validation now derives OAuth-active state exactly as session init, so a bearer token is validated against the same auth config/endpoint on swap as at init (previously an OAuth-active profile with an extra `bearer` authConfig could validate against a different config mid-session). The advertised issuer/origin behind a trusted proxy now uses only the first `X-Forwarded-Host` value (chained proxies append entries) so the issuer is never a malformed comma-separated host; forwarded headers remain honored only when trust-proxy is enabled (deployment requires the fronting proxy to set/overwrite Host and X-Forwarded-Host).
...
### Added
- Consent page customization per deployment: `consent_gate.template_path` supplies a full-page HTML template (mandatory `{{consent_body}}` placeholder for the server-owned form/info/expired block; cosmetic placeholders `{{rules_version}}`/`{{rules_summary}}`/`{{education_resource}}`/`{{title}}`) and `consent_gate.labels` overrides the accept/submit texts. Labels are part of the rules hash (changing them forces re-consent); the template is cosmetic and deliberately not hashed. Form mechanics, CSP and no-store headers stay server-owned; scripts remain blocked. Approval bounds are tunable via `MCP4_CONSENT_APPROVAL_TTL_MS` and `MCP4_CONSENT_CONSUMED_MAX` (AIPP-522).
